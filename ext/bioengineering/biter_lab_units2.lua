...
local small_biter_loot = {
				...
				{type="item", name="PLORD_small_biter_blood", amount_min=100, amount_max=150, probability=0.5},
...
local small_spitter_loot = {
				...
				{type="item", name="PLORD_small_spitter_blood", amount_min=100, amount_max=150, probability=0.5},
...
local biters_tiers = {
	lowtier = {"small", "t1", "t2"},
	midtier = {"medium", "t3", "t4"},
	bigtier = {"big", "t5", "t6"},
	behetier = {"behemoth", "t7", "t8"},
	levitier = {"leviathan", "t9", "t10"},
}
...
function biters_tints_checklist(e_name)
		...
		-- PLORD labs
		elseif e_name:find("PLORD") and e_name:find("megatheria") then tint = {r=0.4, g=0.1, b=0.6, a=1} tint2 = {r=0.3, g=0.25, b=0.3, a=1} end
end

for e_name, cat_lists in pairs(biters_cat_lists) do
	for e_name, entity in pairs(cat_lists) do
		biters_tints_checklist(e_name)
		if (entity.type == "unit-spawner") then return
			elseif e_name:find("biter") and not e_name:find("armoured") then
...
--[[
		if not e_name:find("spawner") then
			if e_name:find("biter") or e_name:find("drone") and not e_name:find("armoured") then 
				entity.icons = generate_biters_icons(tint, tint2, biter_base_icon, biter_base_icon_mask_1, biter_base_icon_mask_2)
			end
...
--[[for e_name, cat_lists in pairs(biters_cat_lists) do
	for e_name, entity in pairs(cat_lists) do
		if not e_name:find("spawner") and e_name:find("corpse") then
    	...
    	if not e_name:find("armoured") and e_name:find("biter") and 
    	(e_name:find biters_tiers[1]) then entity.minable = {mining_time = 0.5, results = small_biter_loot} end
    	if e_name:find("spitter") and 
    	(e_name:find biters_tiers[1]) then entity.minable = {mining_time = 0.5, results = small_spitter_loot} end
		end
	end
end--]]
--
for e_name, entity in pairs(data.raw["corpse"]) do
	...
	if e_name:find("biter") and not e_name:find("armoured")  then
		if e_name:find("small") then entity.minable = {
			mining_time = 0.5,
			results = small_biter_loot
		}
		end
...
artificial_small_biter.order = "a-a"
...
artificial_small_biter.minable = {mining_time = 1, result = "PLORD_artificial_small_biter", count = 1}


megatheria_biter_scale = 10
megatheria_biter_tint = {r=0.4, g=0.1, b=0.6, a=1}
megatheria_biter_tint2 = {r=0.3, g=0.25, b=0.3, a=1}
...
artificial_megatheria_biter.order = "a-e"
artificial_megatheria_biter.max_health = 100000
artificial_megatheria_biter.healing_per_tick = 0.2
artificial_megatheria_biter.vision_distance = 150
artificial_megatheria_biter.minable = {mining_time = 5, result = "PLORD_artificial_megatheria_biter", count = 1}
artificial_megatheria_biter.selection_box = {{-1.4, -1.7}, {1.4, 1.4}}
...
artificial_megatheria_biter.movement_speed = 0.5
...
data:extend({artificial_small_biter, artificial_small_spitter, artificial_small_biter_bad_dna, artificial_megatheria_biter
})
